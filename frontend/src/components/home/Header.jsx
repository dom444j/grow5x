...
const Header = () => {
  ...
  // Navegación para usuarios autenticados (en dropdown)
  const authenticatedNavigation = useMemo(() => {
     ...
     if (isAdmin()) {
       userNavigation.push({ name: 'Panel de Administración', href: '/admin/overview' })
     } else if (isSupport()) {
       userNavigation.push({ name: 'Panel de Soporte', href: '/admin/support' })
     } else {
       userNavigation.push({ name: 'Mi Dashboard', href: '/user/dashboard' })
     }
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden">
            <div className="px-4 pt-4 pb-6 space-y-2 border-t border-gray-200/50 bg-white/95 backdrop-blur-md">
              ...
              {isAuthenticated && authenticatedNavigation.length > 0 && (
